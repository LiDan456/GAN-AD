...
def train_epoch(epoch, samples, labels, sess, Z, X, D_loss, G_loss, D_solver, G_solver,
                ...
                latent_dim, num_generated_features, max_val, one_hot):
    """
...
def create_placeholders(batch_size, seq_length, latent_dim, num_generated_features):
    ...
    T = tf.placeholder(tf.float32, [batch_size, seq_length, num_generated_features])
    return Z, X, T

# def generator(
def generator(z, hidden_units_g, seq_length, batch_size, num_generated_features, reuse=False, parameters=None, learn_scale=True):
...
def dump_parameters(identifier, sess):
    ...
    # dump_path = './experiments/plots/parameters/' + identifier + '.npy'
